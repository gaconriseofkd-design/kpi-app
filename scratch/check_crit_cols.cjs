...
async function checkCritCols() {
  const { data } = await supabase.from('mqaa_patrol_criteria').select('*').limit(1);
  if (data && data.length > 0) {
    console.log('mqaa_patrol_criteria columns:', Object.keys(data[0]));
  }
}
