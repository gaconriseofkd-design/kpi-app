...
function RulesContent() {
  ...
  function addRow() {
    setRows(r => [{ id: undefined, threshold: 100, score: 7, note: "", active: true }, ...r]);
  }
  ...
  async function saveAll() {
    ...
    for (const r of cleaned) {
      const key = String(r.threshold);
      if (uniq.has(key)) return alert("Ngưỡng threshold bị trùng: " + key);
      uniq.add(key);
    }
    setSaving(true);
  ...
  const testScore = useMemo(() => scoreByProductivity(testOE, rows), [testOE, rows]);
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {/* Test nhanh */}
      <div className="p-3 rounded border bg-white inline-flex items-center gap-2">
        <span>Test OE:</span>
        <input type="number" className="input w-28" value={testOE} onChange={e=>setTestOE(Number(e.target.value))}/>
