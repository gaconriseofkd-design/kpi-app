...
try {
    ...
    Add-Type -MemberDefinition '[DllImport("user32.dll")] public static extern bool SetForegroundWindow(IntPtr hWnd);' -Name "Win32ForceReport" -Namespace "UtilForce" -ErrorAction SilentlyContinue
    [UtilForce.Win32ForceReport]::SetForegroundWindow($zaloProcess.MainWindowHandle)
    ...
    # Cap nhat last_patrol_report_monday = ngay cuoi da gui
    if ($lastSuccessDate) {
        $headers = @{
            "apikey"        = $SUPABASE_KEY
            "Authorization" = "Bearer $SUPABASE_KEY"
        ...
        Write-Host ">>> Tu mai, script se tu dong gui bao cao moi ngay luc 08:00 AM" -ForegroundColor Cyan
    }
