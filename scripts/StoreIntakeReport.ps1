...
try {
    ...
    # Cleanup invisible excel processes
    Get-Process Excel -ErrorAction SilentlyContinue | Where-Object { [string]::IsNullOrWhiteSpace(# scripts/StoreIntakeReport.ps1
param(
    [switch]$ManualTrigger,
    [string]$TargetReport = ""
)
...
}.MainWindowTitle) } | Stop-Process -Force
    # Cleanup stranded hidden workbooks in visible excel processes
    try {
